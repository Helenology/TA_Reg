...
# 读入csv文件，fileEncoding代表文件的编码方式
a=read.csv("第4章.csv")
# 设置a的列名
names(a)=c("gender","usage","credit","loan","history","accounts","status")
# 对a增加一列为Y1，当status>0时Y1=1，表示逾期；否则Y1=0，未逾期
a$Y1=1*(a$status>0)
# 对a增加一列为Y2，使Y2等于a原来的status一列
a$Y2=a$status
# 展示a的前五行
a[c(1:5),]
# 展示a的每一列的取值及大小
...
# 取出a中满足Y1列等于1的数据，取名为a1 
a1=a[a$Y1==1,]
# 展示a1的前五行
a1[1:5,]
# 统计a1中Y2列的每个取值下对应的数据量，记为tab
tab=table(a1$Y2)

# 对逾期样本的各种逾期情况分别计算频数，并做柱状图
# 使用数据tab绘制条形图，x轴标题设置成"逾期严重程度"，主标题设置为空
barplot(tab,xlab="逾期严重程度",main=NULL)

# 把过度严重的逾期情况（Y2 >= 4）整合成为一类：将a的Y2列更改为：如果Y2>4则修改为4，如果Y2<=4则保留对应取值
a$Y2=4*(a$Y2>4)+a$Y2*(a$Y2<=4)
# 单独提取出逾期样本，命名为a1: 取出a中满足Y1列等于1的数据，取名为a1，此时的a1会覆盖之前的a1
a1=a[a$Y1==1,]
# 展示a1的前五行
a1[1:5,]
# 统计a1中Y2列的每个取值下对应的数据量，记为tab，此时的tab会覆盖之前的值
tab=table(a1$Y2)
# 使用数据tab绘制条形图，x轴标题设置成"逾期严重程度"，主标题设置为空
barplot(tab,xlab="逾期严重程度",main=NULL)
...
# 计算性别分布情况
# 统计a中gender列的每个取值下对应的数据量，记为tab，此时的tab会覆盖之前的值
tab=table(a$gender)
# 展示tab
tab
# 将tab里每个取值下的数量除以总量，化成每个取值下的频率
tab/sum(tab)


# 对连续型解释变量信用卡使用率做直方图： 绘制a的usage列的直方图，x轴标题为"信用卡相对使用率"，y轴标题为"频数"，主标题设置为空
hist(a$usage,xlab="信用卡相对使用率", ylab = "频数",main=NULL)
# 展示a中usage列取值大于4的数据，肉眼检查使用率大于4的样本
a[a$usage>4,]


# 定义正常使用率（usage=1）和非常正使用率（usage=0）。将a中的usage列大于1的值设置为0，小于等于1的值设置为1
a$usage=1*(a$usage<=1)
# 统计a中usage列的每个取值下对应的数据量，记为tab，此时的tab会覆盖之前的值
tab=table(a$usage)
# 展示tab
tab
# 将tab里每个取值下的数量除以总量，化成每个取值下的频率。 从中可以看到正常组和异常组样本量分别为：4671和3329，它们全样本占比分别为：58.4%和41.6%
tab/sum(tab) 


# 获得授信额度直方图： 绘制a的credit列的直方图，x轴标题为"授信额度（元）"，y轴标题为"频数"，主标题设置为空。
# 从中可以看到授信额度的直方图并不是非常连续。并且该图严重右偏，这说明有少数用户的授信额度很高
hist(a$credit,xlab="授信额度（元）", ylab = "频数",main=NULL)
# # 对a的credit列取对数。 
a$credit=log(a$credit)
# # 绘制a的credit列取对数后的直方图，x轴标题为"对数授信额度"，y轴标题为"频数"，主标题设置为空。 发现分布情况似乎好了很多，整个分布形状更加对称了
hist(a$credit,xlab="对数授信额度", ylab = "频数",main=NULL)


# 定义跟房贷月供相关的两个解释变量。
# 在a中增加一列为Z1，当a的loan列取值大于0则设为1，否则设为0。Z1是一个0-1型哑变量，表示该用户是否有房贷，Z1=1表示有房贷，Z1=0表示没有房贷。
a$Z1=1*(a$loan>0)
# # 在a中增加一列为Z2，将a的loan列的取值复制到Z2中。Z2的非零部分对应的是有房贷用户的房贷金额
a$Z2=a$loan
#对Z2的非零部分取值做对数变换： 将a的loan列大于0的数据的Z2列取对数，然后修改原来的值。 
a[a$loan>0,]$Z2=log(a[a$loan>0,]$Z2)
# 展示a的前5行
a[1:5,]
# 计算房贷比例如下，从中可见有房贷的样本占比为17.0%
mean(a$Z1)
# 取出a中满足Z1列等于1的数据，取名为a2
a2=a[a$Z1==1,]
# 对Z2的非零部分（对应的是有房贷用户）的做直方图。 x轴标题为"对数房贷月供"，y轴标题为"频数"，主标题设置为空
# 从中可以看出一个非常优美的，近似于正态的分布
hist(a2$Z2,xlab="对数房贷月供", ylab = "频数",main=NULL)

# 对历史逾期次数，计算频数并画图。 
# 统计a中history列的每个取值下对应的数据量，记为tab，此时的tab会覆盖之前的值
tab=table(a$history)
# 绘制tab的条形图，x轴标题为"历史逾期次数"，y轴标题为"频数"，主标题设置为空
# 可见这是一个优美的柱状图，拖着长长的向右倾斜的尾巴。从中可以看到，在样本中，有不少人有历史逾期记录。
barplot(tab,xlab="历史逾期次数", ylab = "频数",main=NULL)

# 对信用卡开户数，计算频数并画图
# # 统计a中accounts列的每个取值下对应的数据量，记为tab，此时的tab会覆盖之前的值
tab=table(a$accounts)
# 绘制tab的条形图，x轴标题为"信用卡开户数"，y轴标题为"频数"，主标题设置为空
# 可见该变量的分布情况比较连续。但是，令人有点担忧的是有少数样本取值非常高（最高能到67！）。
...
# 在a的gender的每一个取值（男或女）下计算Y1的均值
mu1=tapply(a$Y1,a$gender,mean)
# 在a的usage的每一个取值（0或1）下计算Y1的均值
mu2=tapply(a$Y1,a$usage,mean)
# 在a的Z1的每一个取值（0或1）下计算Y1的均值
...
# 对剩下的连续型（或者近似连续型）解释变量和Y1做箱线图。 从中可以看到，逾期用户的授信额度偏低，历史逾期次数偏高，而信用卡开户数目似乎差别不大。
# 设置子图为1行3列
par(mfrow=c(1,3))
# 绘制a的Y1取值0和1下的credit变量的箱线图
boxplot(credit~Y1,data=a,ylab="对数授信额度",xlab="是否逾期",names=c("非逾期","逾期"))
# 绘制a的Y1取值0和1下的history变量的箱线图
boxplot(history~Y1,data=a,ylab="历史逾期次数",xlab="是否逾期",names=c("非逾期","逾期"))
# 绘制a的Y1取值0和1下的accounts变量的箱线图
...
# 设置子图为1行1列
par(mfrow=c(1,1))
# 首先筛选出所有的有房贷样本，即取出a中满足loan列大于0的数据，取名为a1
a1=a[a$loan>0,]
# 绘制a1的Y1取值0和1下的Z2变量的箱线图。从下图可以看出逾期用户和非逾期用户之间的对数月供差别似乎并不明显
boxplot(Z2~Y1,data=a1,ylab="对数月供",xlab="是否逾期",names=c("非逾期","逾期"))


#--------------结合Y2（逾期严重程度）对解释变量做描述统计--------------------------
# 由于我们关注的是逾期程度，因此剔除非逾期的正常用户。
# # 取出a中满足Y1列等于1的数据，取名为aa
aa=a[a$Y1==1,]

#结合Y2对三个离散型解释变量做描述统计。 它们分别是：性别（男性、女性），信用卡使用频率（0=不正常，1=正常），还有是否有房贷（有，没有）。

# 定义aa的新的一列为sex，若aa的gender列取值为"男性"则sex取1，否则取0
aa$sex=1*(aa$gender=="男性")

#对Y2的不同逾期严重程度分组计算男性占比，正常使用率占比，房贷占比。 从中可以看到，逾期越严重，男性占比越高，变化范围大概从68%到73%。同时发现，逾期严重程度同信用卡正常使用率占比、房贷占比的关系很混乱，缺乏清晰规律
# 设置子图为1行3列
par(mfrow=c(1,3))
# 计算aa的Y2的每个取值下sex变量的均值，然后绘制折线图
plot(tapply(aa$sex,aa$Y2,mean),type="b",ylab="男性占比",xlab="逾期严重程度",ylim=c(0,1))
# 计算aa的Y2的每个取值下usage变量的均值，然后绘制折线图
plot(tapply(aa$usage,aa$Y2,mean),type="b",ylab="正常使用率占比",xlab="逾期严重程度",ylim=c(0,1))
# 计算aa的Y2的每个取值下Z1变量的均值，然后绘制折线图
...
# 设置子图为2行2列
par(mfrow=c(2,2))
# 绘制aa的Y2列的每个取值下creadit变量的箱线图
boxplot(credit~Y2,data=aa,ylab="对数授信额度",xlab="逾期严重程度")
# 绘制aa的Y2列的每个取值下history变量的箱线图
boxplot(history~Y2,data=aa,ylab="历史逾期次数",xlab="逾期严重程度")
# 绘制aa的Y2列的每个取值下accounts变量的箱线图
boxplot(accounts~Y2,data=aa,ylab="信用卡开户数",xlab="逾期严重程度")

# 取出aa中满足loan列>0的数据，取名为aa1
aa1=aa[aa$loan>0,]
#通过箱线图展示逾期严重程度同对数月供之间的关系
boxplot(Z2~Y2,data=aa1,ylab="对数月供",xlab="逾期严重程度")

#-----------------逾期风险模型： 对预期风险（即：是否逾期）建立0-1回归分析模型。以Y1为因变量，建立回归全模型-----------
# 建立逻辑回归模型，其中因变量为Y1，自变量为gender、usage、credit、Z1、Z2、history、accounts，使用的数据为a
model.full=glm(Y1~gender+usage+credit+Z1+Z2+history+accounts,
               family=binomial(link=logit),data=a)
# 展示模型结果
summary(model.full)

# # 根据模型结果计算卡方统计量的值。 注意空模型（只含有截距项的逻辑回归模型）的离差（deviance）是10665.2，（Null deviance）对应的自由度是：样本量（8000）-1（截距项）=7999。残差的deviance是8924.5，相应的自由度是：样本量（8000）-1（截距项）-解释变量个数（7）=7992。因此，可以做一个关于该模型全局显著性的卡方检验。统计量为：10665.2-8924.5=1740.7，
10665.2-8924.5
# 对应的应该是一个自由度为：7999-7992=7的卡方分布。对应的P-值可以通过下面代码计算得到。
1-pchisq(1740.7,df=7)

#自动寻找最优的AIC模型，trace=F表示不显示选择的过程。与全模型相比，AIC模型放弃了一个解释变量：Z1（是否有月供）。
model.aic=step(model.full,trace=F)
# 展示最优的AIC模型
summary(model.aic)
# 根据模型结果计算卡方统计量的值。 做一个关于该模型全局显著性的卡方检验。统计量为：10665.2-8925.1=1740.1，
10665.2-8925.1
# 对应的应该是一个自由度为：7999-7993=6的卡方分布。
# 对应的P-值可以通过下面代码计算得到。可见非常显著，这说明该模型是显著的。
1-pchisq(1740.1,df=6)

#自动寻找最优的BIC模型，trace=F表示不显示选择的过程。
# 将样本量赋值给ss
ss=length(a[,1])
model.bic=step(model.full,trace=F,k=log(ss))
# 展示最优的BIC模型。可见模型结果跟AIC模型基本类似。唯一区别在于：BIC模型放弃了一个解释变量：信用卡开户数。
summary(model.bic)
...
#--------------------对三个模型做内样本预测代码如下-----------------
# 加载pROC包
# 如果显示不存在这个包需要运行下方代码下载此包
# install.packages("pROC") # 运行完后注释掉
library(pROC)
...
# 打印不同模型对应的auc值。可见三个模型的AUC取值分别为76.77%，76.77%，和76.70%，几乎没有任何差别。因此，BIC模型是一个更好的选择（因为最简单）。
