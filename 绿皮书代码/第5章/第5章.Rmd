...
# 将分词结果出现频次最高的100个词赋值给tab2
tab2=tab[1:100]
# 调整tab2中的频次为频率
tab2=tab2/sum(tab)
tab2
# # 计算tab2中词的总频率。这些词根虽然只有100个，但是却在总的词根频数中占比接近95%
sum(tab2)
# 将a的样本量赋值给ss
ss=length(a[,1])
...
# 展示转化量过高（Y2 >60）的关键词。为检验数是否有异常值,提取出大量转化（Y2 >60）的关键词数据。
...
# 设置子图为1行3列
par(mfrow=c(1,3))
# 绘制Y1各取值下对数展现量、对数点击率、对数单位点击成本的箱线图
...
# 在a的keyword中找"多少钱"出现的位置；conv1表示：首先取出a的pos位置的数据，conversion>0出现的次数；len1表示pos的个数
pos=grep("多少钱",a$keyword);conv1=sum(a[pos,]$conversion>0);len1=length(pos)
# 在a的keyword中找"哪个"出现的位置；conv2表示：首先取出a的pos位置的数据，conversion>0出现的次数；len2表示pos的个数
pos=grep("哪个",a$keyword);conv2=sum(a[pos,]$conversion>0);len2=length(pos)
# 展示以下四个变量
c(conv1,conv2,len1,len2)
...
# 在a的keyword中找"多少钱"出现的位置；tmp表示：提取出a的pos位置的数据；tmp1表示取出tmp中conversion变量大于0的conversion列
pos=grep("多少钱",a$keyword);tmp=a[pos,];tmp1=tmp[tmp$conversion>0,]$conversion;
# 在a的keyword中找"哪个"出现的位置；tmp表示：提取出a的pos位置的数据；tmp1表示取出tmp中conversion变量大于0的conversion列
pos=grep("哪个",a$keyword);tmp=a[pos,];tmp2=tmp[tmp$conversion>0,]$conversion;

# 展示tmp1的均值，样本量，tmp2的均值，样本量
c(mean(tmp1),length(tmp1),mean(tmp2),length(tmp2))

# 提取各变量的字符串长度
a$kwlen=nchar(a$keyword)
a$ticket=nchar(a$ticket)
a$price=nchar(a$price)
a$buy=nchar(a$buy)
a$city1=nchar(a$city1)
a$city2=nchar(a$city2)
a$brand=nchar(a$brand)
# 展示前5行
a[1:5,]
...
# 将aa赋值给aa2
aa2 = aa
# 在aa2中存储aa中ranking和CPC的原始数据
aa2$orirank = aa$ranking
aa2$oriCPC = aa$CPC
#控制展现量为1万次展现，将aa2的一列impression设置为10000
aa2$impression = 10000
#点击率为1%，将aa2的一列CTR设置为1
aa2$CTR = 1
#点击成本为1元/点击，将aa2的一列CPC设置为1
aa2$CPC = 1
#竞价排名为第1名，将aa2的一列ranking设置为1
aa2$ranking = 1
#预测得“理论转化量”。使用BIC选模型预测，赋值给aa2的p2列
aa2$p2 = predict(poisson.bic,newdata = aa2)
# 根据"理论转化量"的中位数，可以将所有关键词区分为两类：优质、劣质。
aa2$pc2 = cut(aa2$p2,
              breaks = c(0,median(aa2$p2),max(aa2$p2)+1),
              labels=c("劣质","优质"))
# 根据排名的前后，将排名前七的分为排名靠前，其他的分为排名靠后。
aa2$rankingc = cut(aa2$orirank,
                   breaks = c(0,7,max(aa2$orirank)+1),
                   labels=c("排名靠前","排名靠后"))

#四类关键词的转化成本，对比这四类关键词的平均一次转化产生的成本（单位转化成本，CPA）。
# 提取aa2中排名靠前的优质关键词所在的行，并另存为新的数据集new1
new1 = aa2[(aa2$pc2 %in% "优质")&(aa2$rankingc %in% "排名靠前"),]
# 提取aa2中排名靠前的劣质关键词所在的行，并另存为新的数据集new2
new2 = aa2[(aa2$pc2 %in% "劣质")&(aa2$rankingc %in% "排名靠前"),]
# 提取aa2中排名靠后的优质关键所在的行，并另存为新的数据集new3
new3 = aa2[(aa2$pc2 %in% "优质")&(aa2$rankingc %in% "排名靠后"),]
# 提取aa2中排名靠后的优质关键词所在的行，并另存为新的数据集new4
...
# 这里我们关心排名靠前的劣质关键词，因为它们浪费了太多的营销资源，而没有产生相匹配的效果。
# 提取aa2中排名靠前的劣质关键词所在的行，并另存为新的数据集aa3
aa3 = aa2[(aa2$pc2 %in% "劣质")&(aa2$rankingc %in% "排名靠前"),]
# 提取aa2中所有排名靠前的关键词所在的行，并另存为新的数据集aa30
aa30 = aa2[(aa2$rankingc %in% "排名靠前"),]
#计算在全部排名靠前的关键词中，劣质关键词的占比
m1 = nrow(aa3)/nrow(aa30) 
#计算排名靠前的劣质关键词的平均排名
m2 = mean(aa3$orirank) 
#计算排名靠前的劣质关键词的消耗的营销费用在全部排名靠前关键词的营销费用中的占比
m3 = sum(aa3$cost)/sum(aa30$cost) 
#计算排名靠前的劣质关键词的转化量在全部排名靠前关键词的转化量的占比
m4 = sum(aa3$conversion)/sum(aa30$conversion) 
# 在全部排名靠前的关键词中，这部分关键词占比53.94%，平均排名大概第5名，消耗了7.33%的营销费用。
cat(m1,m2,m3,m4)

# 这里我们关心排名靠后的优质关键词，它们的营销效果没有得到充分发挥。对于这部分关键词，可以考虑试探性增加出价，提高排名，以期获得更好的转化效果。
# 提取aa2中排名靠后的优质关键词所在的行，并另存为新的数据集aa4
aa4 = aa2[(aa2$pc2 %in% "优质")&(aa2$rankingc %in% "排名靠后"),]
# 提取aa2中所有排名靠后的关键词所在的行，并另存为新的数据集aa40
aa40 = aa2[(aa2$rankingc %in% "排名靠后"),]
#计算在全部排名靠后的关键词中，优质关键词的占比
m1 = nrow(aa4)/nrow(aa40) 
#计算排名靠后的优质关键词的平均排名
m2 = mean(aa4$orirank) 
#计算排名靠后的优质关键词的消耗的营销费用在全部排名靠后的关键词营销费用中的占比
m3 = sum(aa4$cost)/sum(aa40$cost)
#计算排名靠后的优质关键词的转化量在全部排名靠后关键词的转化量的占比
m4 = sum(aa4$conversion)/sum(aa40$conversion) 
# 在全部排名靠后的关键词中，这部分关键词占比50.82%，平均排名大概为第9名，排名相对靠后，消耗了44.79%的营销费用。
cat(m1,m2,m3,m4)
```
